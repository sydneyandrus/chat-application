...
@auth_router.post("/registration", status_code=201, response_model=UserResponse)
def register_new_user(
  ...
  """Register new user."""
  check_username = session.exec(
      select(UserInDB).where(UserInDB.username == registration.username)
    ).first()
  
  if check_username is not None:
    raise UserException("username", registration.username)
  
  check_email = session.exec(
      select(UserInDB).where(UserInDB.email == registration.email)
    ).first()
  
  if check_email is not None:
    raise UserException("email", registration.email)
